...
// Connect Es 初始化并返回一个配置好的 Elasticsearch 客户端
func ConnectEs() *elasticsearch.TypedClient {
	esCfg := global.Config.ES
	cfg := elasticsearch.Config{
		Addresses: []string{esCfg.URL}, // 这就是集群吗？
	...
	if esCfg.IsConsolePrint {
		// cfg.logger为接口类型
		cfg.Logger = &elastictransport.ColorLogger{ // 有点明白了，*ColorLogger实现了logger里面的所有的方法，继承了该接口，要调用就必须用指针
