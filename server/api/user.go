...
func (userApi *UserApi) Login(c *gin.Context) {
	switch c.Query("flag") {
	case "email":
		userApi.EmailLogin(c) // 邮箱登录
	case "qq":
		userApi.QQLogin(c) // qq登录
	default:
...
func (userApi *UserApi) EmailLogin(c *gin.Context) {
	var req request.Login // 不懂为啥登录的时候必须要有个验证码呢
	err := c.ShouldBindJSON(&req)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
...
func (userApi *UserApi) QQLogin(c *gin.Context) {
	...
	if err != nil || accessTokenResponse.Openid == "" { //  获取访问令牌失败, 返回错误,不加accessTokenResponse.Openid会有个严重的bug
