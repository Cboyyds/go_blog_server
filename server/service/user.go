...
func (userService *UserService) UserWeather(ip string) (string, error) {
	...
	if err != nil {
		ipResponse, err := ServiceGroupApp.GaodeService.GetLocationByIP(ip) // 先去拿到ip
		if err != nil {
			return "", err
		}
		live, err := ServiceGroupApp.GaodeService.GetWeatherByAdcode(ipResponse.Adcode) // 再根据ip和code取出天气情况
		if err != nil {
			return "", err
		}
