...
func Run(c *cli.Context) {
	// 检查是否设置了多个标志
	fmt.Println("数量是几啊", c.NumFlags(), c)
	if c.NumFlags() > 1 {
...
// InitFlag 初始化命令行参数并执行应用程序。
// 该函数检查命令行参数，如果存在参数，则创建并运行应用程序。
// 如果应用程序执行过程中发生错误，将记录错误并退出程序。
// 如果第一个参数是 "-h" 或 "-help"，则显示帮助信息并退出程序。
func InitFlag() {
	// 检查是否有命令行参数
	if len(os.Args) > 1 {
		// 创建新的应用程序实例
