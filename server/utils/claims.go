...
func SetRefreshToken(c *gin.Context, token string, maxAge int) {
	...
	setCookie(c, "x-refresh-token", token, maxAge, host) // host是主机名，IP ，port是端口号
}
...
func setCookie(c *gin.Context, name, value string, maxAge int, host string) {
	...
	if net.ParseIP(host) != nil {
		// 如果是IP地址，设置cookie的domain为“/” ，因为Cookie的  Domain  属性不支持直接设置为IP地址，只能设置为  "/"
		c.SetCookie(name, value, maxAge, "/", "", false, true) // 这里没有ssl证书于是要用secure：false
	} else {
...
func GetUserInfo(c *gin.Context) *request.JwtCustomClaims {
	...
	if claims, exists := c.Get("claims"); !exists {
		...
		// 如果已存在claims，则直接返回
		waitUse := claims.(*request.JwtCustomClaims) // 进行一个类型断言
		return waitUse
	}
}
...
func GetRoleID(c *gin.Context) appTypes.RoleID {
	...
	if claims, exists := c.Get("claims"); !exists {
		// 如果不存在，则重新解析Access Token
		if cl, err := GetClaims(c); err != nil { // 是从accessToken中解析的
			// 如果解析失败，返回0
			return 0
		} else {
			// 返回解析出来的角色ID
